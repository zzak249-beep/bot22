...
# ══════════════════════════════════════════════════════════
# BACKTEST v5.2 FINAL — LONG + SHORT
# FIX RAIZ: API devuelve "open","high","low","close","volume","time"
#           El codigo anterior usaba "o","h","l","c","v","t" → todo 0
# ══════════════════════════════════════════════════════════
...
def fetch(sym):
    ...
    for c in all_c:
        if isinstance(c, list):
            ...
            # FIX: API devuelve nombres completos, no abreviados
            rows.append([
    ...
    if df.empty or df["close"].max() == 0:
        print(" sin datos válidos"); return pd.DataFrame()
...
def backtest(df, sym, balance):
    ...
    for i in range(BB_PERIOD + 30, len(df)):
        ...
        touch_long = (price <= blo * 1.003) or \
                     (dv == "bull" and r < 50 and price <= blo * 1.015)
        ...
        touch_short = (price >= bhi * 0.997) or \
                      (dv == "bear" and r > 55 and price >= bhi * 0.985)
...
def report(all_trades, final_bal):
    ...
    out = "\n".join(lines)
    print(out)
    with open("backtest_resultado_v5.txt", "w", encoding="utf-8") as f:
        f.write(out + f"\n\nGenerado: {datetime.now()}\n")
    print("\n  Guardado en: backtest_resultado_v5.txt")


def main():
    print("=" * 62)
    print("  BACKTEST v5.2 FINAL — LONG + SHORT — BingX 1h")
