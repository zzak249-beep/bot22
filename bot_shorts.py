"""
Bot SHORT-Only Multi-Symbol — v2.2
Mismos fixes que bot.py v4.2 aplicados al bot de shorts.
"""
...
MIN_QUALITY     = _senv("MIN_QUALITY",       "60",      int)
ENTRY_MAX_PROB  = _senv("ENTRY_MAX_PROB",    "0.45",    float)
...
MIN_BALANCE     = _senv("MIN_BALANCE",      "15",      float)
POSITION_MODE   = _senv("POSITION_MODE",    "auto")
REPORT_EVERY    = _senv("REPORT_EVERY",     "12",      int)
MAX_OPEN_TRADES = _senv("MAX_OPEN_TRADES",  "2",       int)
MIN_VOLUME_24H  = _senv("MIN_VOLUME_24H",   "8000000", float)
MAX_SYMBOLS     = _senv("MAX_SYMBOLS",      "15",      int)
SCAN_INTERVAL   = _senv("SCAN_INTERVAL",   "600",     int)
COOLDOWN_MIN    = _senv("COOLDOWN_MIN",    "120",     int)
USE_LIMIT       = _senv("USE_LIMIT_ORDERS","true").lower() == "true"
MIN_HOLD_MIN    = _senv("MIN_HOLD_MIN",    "25",      int)
MIN_ATR_PCT     = _senv("MIN_ATR_PCT",     "0.7",     float)
USE_BTC_FILTER  = _senv("USE_BTC_FILTER", "true").lower() == "true"
MAX_LOSS_STREAK = _senv("MAX_LOSS_STREAK", "2",       int)
MAX_TRADES_DAY  = _senv("MAX_TRADES_DAY",  "4",       int)
MIN_PROFIT_USDT = _senv("MIN_PROFIT_USDT", "0.40",    float)
MAX_PUMP_PCT    = _senv("SH_MAX_PUMP_PCT", "4.0",     float)
MIN_DROP_PCT    = _senv("SH_MIN_DROP_PCT", "0.5",     float)
...
def update_blacklist(symbol, win, pnl_net=0.0):
    ...
    if not win and pnl_net < -0.50:
        set_cooldown(symbol, COOLDOWN_MIN * 4)
...
def retry_tp_sl_if_missing(symbol):
    ...
    if t.get("has_tp_sl") or not t.get("tp_price") or not t.get("sl_price"):
        return
    log.info(f"  [TP/SL RETRY] SHORT {symbol}…")
    tp_sl = client.place_tp_sl(
        symbol, "short", t["entry_qty"], t["tp_price"], t["sl_price"],
        position_side=t["position_side"]
    )
    if tp_sl["tp"] and tp_sl["sl"]:
        open_trades[symbol]["has_tp_sl"] = True
        client.send_telegram(f"<b>✅ TP/SL SHORT reintentados: {symbol}</b>")
...
def handle_short_entry(symbol, signals, balance, df):
    ...
    try:
        ...
        q_score = signals.get("quality_bear", 0)
        tp_icon = "✅" if tp_sl["tp"] else "❌"
        sl_icon = "✅" if tp_sl["sl"] else "❌"

        client.send_telegram(
            f"<b>🔴 SHORT ABIERTO [v2.2]</b>\n"
            ...
            f"RSI: {signals.get('rsi','?')} | BTC 1h: {btc_trend_1h:+.2f}%\n"
...
def send_report(balance):
    ...
    for sym, t in open_trades.items():
        ...
        pos_lines += f"  SHORT {sym}: {pct:+.2f}% TP/SL:{tp_sl}\n"
    no_pos = "  sin posiciones\n"
    client.send_telegram(
        f"<b>📊 Reporte Short Bot v2.2 #{stats['cycle']}</b>\n"
...
def main():
    ...
    log.info("=" * 70)
    log.info(f"  Short-Only Bot v2.2 | TF:{TIMEFRAME} LEV:{LEVERAGE}x Max:{MAX_OPEN_TRADES}")
    log.info(f"  LIMIT:{'ON' if USE_LIMIT else 'OFF'} | TP:{TP_PCT}% SL:{SL_PCT}%")
    log.info(f"  Entry prob<{ENTRY_MAX_PROB:.0%} score>={MIN_QUALITY} | MaxDay:{MAX_TRADES_DAY}")
    ...
    client.send_telegram(
        f"<b>🔴 Short Bot v2.2 iniciado</b>\n"
        f"TF:{TIMEFRAME} | LEV:{LEVERAGE}x | Max:{MAX_OPEN_TRADES} | MaxDay:{MAX_TRADES_DAY}\n"
        f"LIMIT:{'✅ 0.02%' if USE_LIMIT else '⚠️'} | MinProfit:${MIN_PROFIT_USDT}\n"
    ...
    while True:
        try:
            ...
            client.send_telegram(f"<b>⚠️ Error Short Bot v2.2</b>\n{e}")
            time.sleep(30)
