...
# ══════════════════════════════════════════════════════
# main.py v13.1 — Diagnóstico de balance en arranque
# ══════════════════════════════════════════════════════
...
def _check_balance_on_start() -> float:
    """
    Verifica balance al arrancar y da diagnóstico claro.
    No bloquea el bot — permite operar aunque balance=0 (paper).
    """
    ...
    if TRADE_MODE == "live":
        if balance > 0:
            ...
            print(f"        → BingX: API Mgmt → crear nueva con Read+Trade")
            print(f"     3) ¿Quieres paper trading? → TRADE_MODE=paper")
            tg.notify_error(
                f"🚨 BOT ARRANCADO pero Balance Futures=$0\n"
                f"El bot detectará señales pero NO ejecutará trades.\n"
                f"→ Transfiere fondos a tu cuenta Perpetual Futures en BingX\n"
                f"→ O activa TRADE_MODE=paper para simular"
            )
    else:
        from config import INITIAL_BAL
        print(f"  📄 PAPER MODE — Balance: ${balance:.2f} (INITIAL_BAL=${INITIAL_BAL})")
        if balance <= 0:
            print(f"  ⚠️  INITIAL_BAL=0 en Railway Variables → ponlo en 100 o más")
    print(f"{'='*56}")
    return balance
...
def run_cycle(cycle: int):
    ...
    if balance <= 0 and TRADE_MODE == "live":
        print(f"  ⚠️  Balance=0 → señales detectadas pero trades bloqueados")
    print(f"{'='*56}")
    ...
    for sym in SYMBOLS:
        try:
            ...
            if sig:
                ...
                print(f"🚀 SEÑAL {sig['side'].upper()} score={sig['score']} "
