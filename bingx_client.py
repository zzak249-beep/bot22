"""
BingX Client FINAL
- Firma correcta (sin sort, sin urlencode)
- Klines: parseo robusto formato v3
- Session pool controlado (max 8 conexiones)
"""
...
class BingXClient:
    ...
    def _req(self,method,path,params):
        ...
        try:
            ...
            r.raise_for_status()
            d=r.json()
            if d.get("code",0)!=0: raise RuntimeError(f"BingX [{d.get('code')}]: {d.get('msg')}")
            return d
        except RuntimeError: raise
        except Exception as e: raise RuntimeError(f"HTTP error: {e}")

    def get_klines(self,symbol,interval="3m",limit=200):
        """Devuelve lista de dicts {timestamp,open,high,low,close,volume}"""
        ...
        if isinstance(first,(list,tuple)):
            ...
            km={"time":"timestamp","t":"timestamp","o":"open","h":"high","l":"low","c":"close","v":"volume"}
            for row in raw:
                try:
                    n={km.get(k,k):v for k,v in row.items()}
                    out.append({"timestamp":int(n.get("timestamp",0)),"open":float(n.get("open",0)),
                                "high":float(n.get("high",0)),"low":float(n.get("low",0)),
                                "close":float(n.get("close",0)),"volume":float(n.get("volume",0))})
                except: continue
        return sorted(out,key=lambda x:x["timestamp"])

    def get_ticker(self,symbol):
        return self._req("GET","/openApi/swap/v2/quote/ticker",{"symbol":symbol}).get("data",{})

    def get_balance(self):
        return self._req("GET","/openApi/swap/v2/user/balance",{}).get("data",{}).get("balance",{})
    ...
    def place_market_order(self,symbol,side,pos_side,quantity,reduce_only=False):
        ...
        p={"symbol":symbol,"side":side,"positionSide":pos_side,"type":"MARKET","quantity":quantity}
    ...
    def place_stop_loss(self,symbol,side,pos_side,stop_price,qty):
        ...
        return self._req("POST","/openApi/swap/v2/trade/order",
                         {"symbol":symbol,"side":side,"positionSide":pos_side,
                          "type":"STOP_MARKET","stopPrice":stop_price,"quantity":qty,"reduceOnly":"true"})

    def place_take_profit(self,symbol,side,pos_side,stop_price,qty):
        if self.demo: return {}
        return self._req("POST","/openApi/swap/v2/trade/order",
                         {"symbol":symbol,"side":side,"positionSide":pos_side,
                          "type":"TAKE_PROFIT_MARKET","stopPrice":stop_price,"quantity":qty,"reduceOnly":"true"})

    def cancel_all_orders(self,symbol):
        if self.demo: return {}
        return self._req("DELETE","/openApi/swap/v2/trade/allOpenOrders",{"symbol":symbol})
