"""
╔══════════════════════════════════════════════════════════════╗
║        BINGX FUTURES ARBITRAGE BOT v2.0 — Railway Ready     ║
...
QUOTE_CURRENCY  = "USDT"
LEVERAGE        = int(os.getenv("LEVERAGE", "1"))   # 1x por defecto en arb (no necesita más)
...
class Bot:
    ...
    async def execute(self, opp: Opportunity) -> bool:
        ...
        try:
            ...
            # En BingX futuros los pares son BTC/USDT:USDT
            def sym(base, quote):
                return f"{base}/{quote}:{quote}"

            # Orden 1: USDT → B (long)
            p1  = sym(path[1], path[0])
            qty1 = amt / self.prices[f"{path[1]}/{path[0]}"]
            o1  = await ex.create_market_buy_order(p1, qty1)

            # Orden 2: B → C
            p2  = sym(path[2], path[1])
            qty2 = o1["filled"] / self.prices[f"{path[2]}/{path[1]}"]
            o2  = await ex.create_market_buy_order(p2, qty2)

            # Orden 3: C → USDT (cierra con sell)
    ...
    async def run(self):
        ...
        await self.tg.send(
            ...
            f"Exchange: *BingX Futuros Perpetuos*\n"
        ...
        try:
            ...
            log.info("✅ Mercados BingX Futuros cargados — escaneando...")

            # Configurar leverage para todos los pares
            if not DRY_RUN and LEVERAGE > 1:
                for b in BASE_CURRENCIES:
                    try:
                        sym = f"{b}/{QUOTE_CURRENCY}:{QUOTE_CURRENCY}"
                        await self.ex.set_leverage(LEVERAGE, sym)
                    except Exception:
                        pass
                log.info(f"✅ Leverage configurado: {LEVERAGE}x")
