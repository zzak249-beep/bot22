...
class RiskManager:
    ...
    def register(self, pnl: float):
        self.daily_pnl += pnl
        self.trades_hr.append(time.time())

    def can_trade(self) -> tuple:
        self._reset_if_new_day()
        cutoff = time.time() - 3600
        while self.trades_hr and self.trades_hr[0] < cutoff:
            self.trades_hr.popleft()
        ...
        if self.daily_pnl <= -MAX_DAILY_LOSS_USDT:
            ...
            return False, self.pause_reason
        if len(self.trades_hr) >= MAX_TRADES_PER_HOUR:
            return False, f"Límite {MAX_TRADES_PER_HOUR} trades/hora"
        return True, ""

    def summary(self) -> str:
        return (
            f"PnL hoy: ${self.daily_pnl:+.4f} | "
            f"Trades/h: {len(self.trades_hr)}/{MAX_TRADES_PER_HOUR}"
        )
...
class Bot:
    ...
    async def run(self):
        log.info(
            ...
            f"  Max trades/h : {MAX_TRADES_PER_HOUR}\n"
