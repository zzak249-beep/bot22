...
# ── Config from ENV ───────────────────────────────────────────────────
# FIXED: support both BINGX_API_SECRET and BINGX_SECRET_KEY naming conventions
...
class SuperBot:
    ...
    def _open_trade(self, symbol: str, signal: Signal):
        ...
        try:
            ...
            # Commission saving: LIMIT order (maker fee 0.02% vs taker 0.05%)
            if LIMIT_ENTRY:
    ...
    def _manage_positions(self):
        ...
        # Get current positions from exchange (skip in dry-run)
        exchange_positions = {}
        if not DRY_RUN:
            try:
        ...
        for symbol, trade in list(active_trades.items()):
            ...
            # Check if exchange closed position (SL hit, liquidated, etc.)
            ...
            if not tp1_hit:
                ...
                if tp1_reached:
                    ...
                    self.risk.record_pnl(est_pnl * 0.5)
                    notifier.notify_tp_hit(symbol, 1, price, partial_qty, est_pnl * 0.5)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Daily summary every 24h (approximate via cycle count)
