"""
╔══════════════════════════════════════════════════════════════════╗
║     BINGX FUTURES ARBITRAGE BOT v3.0 — Railway Ready            ║
...
class Bot:
    ...
    async def execute(self, opp: Opportunity) -> bool:
        ...
        try:
            ...
            qty1 = opp.trade_amt / self.prices[f"{path[1]}/{path[0]}"]
    ...
    async def run(self):
        ...
        await self.tg.send(
            f"🤖 *Bot v3.0 iniciado* — BingX Futuros\n"
                ...
                for b in BASE_CURRENCIES:
                    try:
                        await self.ex.set_leverage(
                            LEVERAGE, f"{b}/{QUOTE_CURRENCY}:{QUOTE_CURRENCY}"
                        )
                    except Exception:
                        pass
