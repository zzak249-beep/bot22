...
# ══════════════════════════════════════════════════════════
# BACKTEST v6 — Fix SL demasiado ajustado
#
# DIAGNOSTICO v5.2:
#   SL 303tr WR:16% → precio toca SL antes de moverse
#   TP+SIGNAL 115tr WR:100% → cuando llega al TP gana siempre
#
# CAUSA: SL=1.5xATR muy ajustado, precio con ruido normal
#        toca el SL y rebota exactamente donde queríamos
#
# FIXES v6:
#   ✅ SL_ATR = 3.0  → más espacio para respirar
#   ✅ RSI_LONG = 38  → solo entrar longs en sobreventa real
#   ✅ SCORE_MIN = 48 → entradas de más calidad (menos trades)
#   ✅ MIN_RR = 1.5   → solo si la ganancia potencial vale el riesgo
#   ✅ COOLDOWN = 4   → evitar entradas en rachas malas
# ══════════════════════════════════════════════════════════

BB_PERIOD      = 20
BB_SIGMA       = 2.0
RSI_PERIOD     = 14
RSI_LONG       = 38       # FIX: era 50 → solo sobreventa real
RSI_SHORT      = 62       # FIX: era 55 → solo sobrecompra real
SL_ATR         = 3.0      # FIX: era 1.5 → más espacio
PARTIAL_TP_ATR = 2.5
LEVERAGE       = 2
RISK_PCT       = 0.02
INITIAL_BAL    = 100.0
SCORE_MIN      = 48       # FIX: era 35 → más selectivo
COOLDOWN_BARS  = 4        # FIX: era 2 → más pausa entre trades
MIN_RR         = 1.5      # FIX: era 1.0 → exigir mejor R:R
...
def backtest(df, sym, balance):
    ...
    for i in range(BB_PERIOD + 30, len(df)):
        ...
        # ══ SEÑAL LONG — solo sobreventa real ═══════════
        ...
        # ══ SEÑAL SHORT — solo sobrecompra real ══════════
...
def report(all_trades, final_bal):
    ...
    out = "\n".join(lines)
    print(out)
    with open("backtest_resultado_v6.txt", "w", encoding="utf-8") as f:
        f.write(out + f"\n\nGenerado: {datetime.now()}\n")
    print("\n  Guardado en: backtest_resultado_v6.txt")


def main():
    print("=" * 62)
    print("  BACKTEST v6 — LONG + SHORT — BingX 1h (~83 dias)")
