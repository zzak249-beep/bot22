...
================================================
Implementación propia, inspirada en el concepto general de "order
blocks ponderados por volumen" — no es traducción de ningún script de
terceros. Combina building blocks genéricos de TA (pivotes, ATR
Wilder, ratio de volumen alcista/bajista).

1. Tendencia: banda ATR alrededor de hl2, flip cuando el precio la cruza.
...
4. Señal: retest de la zona con la tendencia todavía activa Y el
   ratio de volumen por encima de un mínimo configurable.
5. SL: extremo opuesto de la zona; TP: R múltiplo configurable.

Sin backtesting propio, sin track record — punto de partida razonado,
no estrategia validada.
"""
import logging
...
def get_signal(candles: list, config) -> dict:
    """
    candles: velas de entrada, sorted oldest→newest.
