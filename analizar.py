"""
analizar_v8.py — SMC Bot v8.0 [RENTABLE-FINAL]
===============================================
Basado en bt_v4.py — única config rentable probada con datos reales.

CAMBIO FUNDAMENTAL:
  TP = precio ± dist_SL × TP_DIST_MULT (1.2x)
  NO TP = ATR × mult (inalcanzable)

RESULTADO bt_v4 ganador:
  TP=1.2x dist, score≥4 → PnL=+$70.59, WR=55.6%, PF=1.40
"""
...
def es_trending(candles: list, n: int = 20) -> bool:
    """Choppiness Index - más permisivo"""
    if len(candles) < n + 1:
        return True
    w = candles[-(n+1):]
    s = sum(
        max(w[i]["high"] - w[i]["low"], abs(w[i]["high"] - w[i-1]["close"]), abs(w[i]["low"] - w[i-1]["close"]))
        for i in range(1, len(w))
    )
    rng = max(c["high"] for c in w) - min(c["low"] for c in w)
    chop = (s / rng / n * 100) if rng > 0 else 100
    return chop < 68  # Relajado de 61.8 a 68
...
def volumen_ok(candles: list) -> bool:
    ...
    return avg <= 0 or candles[-1]["volume"] / avg >= 0.30  # Relajado de 0.40 a 0.30


# ══════════════════════════════════════════════════════════════
# MTF (más permisivo - NEUTRAL permite operar)
# ══════════════════════════════════════════════════════════════
...
def detectar_fvg(candles: list) -> dict:
    r = {"bull_fvg": False, "bear_fvg": False, "fvg_top": 0.0, "fvg_bottom": 0.0, "fvg_rellenado": False, "en_zona": False}
    if len(candles) < 3:
        return r
    precio = candles[-1]["close"]
    for i in range(len(candles) - 1, max(len(candles) - 20, 2) - 1, -1):
        ...
        if gap_up > config.FVG_MIN_PIPS:
            ...
            r.update({
                "bull_fvg": not mitigado,
                "fvg_top": top,
                "fvg_bottom": bot,
                "fvg_rellenado": mitigado,
                "en_zona": en_zona
            })
            break
...
def detectar_sweep(candles: list) -> dict:
    ...
    if actual["low"] < min_p and actual["close"] > min_p:
        r["sweep_bull"] = True
    if actual["high"] > max_p and actual["close"] < max_p:
        r["sweep_bear"] = True
    return r
...
# ══════════════════════════════════════════════════════════════
# SL ESTRUCTURAL (CRÍTICO - basado en swing, no ATR)
# ══════════════════════════════════════════════════════════════
...
def _calcular_sl_estructural(candles: list, ob: dict, lado: str, atr: float, precio: float) -> float:
    ...
    ATR solo como floor mínimo para evitar ruido.
    """
    if lado == "LONG":
        ...
        sl_min = precio - atr * 0.5  # no más cerca de 0.5 ATR
...
# ══════════════════════════════════════════════════════════════
# TP BASADO EN DIST_SL — CAMBIO CRÍTICO v8.0
# ══════════════════════════════════════════════════════════════

def _calcular_tp(precio: float, sl: float, lado: str) -> tuple:
    """
    TP = precio ± dist_SL × TP_DIST_MULT

    CRÍTICO: este es el cambio que hace el bot rentable según bt_v4.
...
# ══════════════════════════════════════════════════════════════
# SEÑAL PRINCIPAL v8.0 — Más permisiva, TP alcanzable
# ══════════════════════════════════════════════════════════════
...
def analizar_par(par: str):
    try:
        ...
        rsi_l = 20 <= rsi <= config.RSI_BUY_MAX  # Relajado de 25 a 20
        rsi_s = config.RSI_SELL_MIN <= rsi <= 80  # Relajado de 75 a 80
        ...
        # HTF flexible: NEUTRAL permite operar - solo BULL/BEAR explícito bloquea el contrario
        ...
        # ── SCORING v8.0 (max 16, más permisivo) ─────────────────────────
        sl = ss = 0
        ml: list = []
        ms: list = []
        ...
        # CAMBIO CRÍTICO: el backtest NO requería killzone obligatoria
